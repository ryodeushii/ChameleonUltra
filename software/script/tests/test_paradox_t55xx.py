...
from chameleon_com import Response  # noqa: E402
...
EXPECTED_WRITE_PAYLOAD = TEST_ID + bytes.fromhex(
    "20206666 51243648 19920427"
)
...
class MockDevice:
    ...
    def send_cmd_sync(self, command, data=None, **kwargs):
        self.calls.append((command, data))
        if command == Command.GET_DEVICE_MODE:
            return Response(command, Status.SUCCESS, b"\x01")
        if command == Command.GET_DEVICE_MODEL:
            return Response(command, Status.SUCCESS, b"\x00")
        return Response(command, Status.LF_TAG_OK)
...
class TestParadoxT5577Command(unittest.TestCase):
    def test_command_id_is_next_unused_and_matches_header(self):
        header = (Path(__file__).resolve().parents[3] /
                  "firmware" / "application" / "src" / "data_cmd.h").read_text()
